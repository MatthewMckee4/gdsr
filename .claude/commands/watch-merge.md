...
## allowed-tools: Bash(gh pr checks:*), Bash(gh pr merge:*), Bash(gh run view:*), Bash(git pull:*) description: Watch PR checks, fix failures, and squash merge
...
1. If all checks pass, squash merge the PR with `gh pr merge <number> --squash --delete-branch`, then `git pull`
...
1. Repeat until all checks pass and the PR is merged
