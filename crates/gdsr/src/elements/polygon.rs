...
mod tests {
    ...
    #[test]
    fn test_regular_polygon_shapes() {
        let origin = Point::float(0.0, 0.0, 1e-6);

        let triangle = Polygon::regular_polygon(
            Point::float(5.0, 5.0, 1e-6),
            10.0,
            3,
            0.0,
            Layer::new(2),
        ...
        insta::assert_debug_snapshot!(triangle, @"
        Polygon {
            points: [
                Point {
                    x: Float(
                ...
                Point {
                    x: Float(
                        FloatUnit {
                            value: -4.440892098500626e-15,
                            units: 1e-6,
                        },
                    ),
                    y: Float(
                        FloatUnit {
                            value: -3.6602540378443837,
                            units: 1e-6,
                        },
                    ),
                },
        ...
        let square = Polygon::regular_polygon(origin, 1.0, 4, 0.0, Layer::new(0), DataType::new(0));
        ...
        let hexagon =
            Polygon::regular_polygon(origin, 10.0, 6, 0.0, Layer::new(1), DataType::new(0));
        insta::assert_debug_snapshot!(hexagon, @"
        Polygon {
            points: [
                Point {
                    x: Float(
    ...
    #[test]
    fn test_regular_polygon_rotation_and_clamping() {
        let origin = Point::float(0.0, 0.0, 1e-6);
        ...
        insta::assert_debug_snapshot!(rotated, @"
        Polygon {
            points: [
                Point {
                    x: Float(
